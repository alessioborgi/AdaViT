...
def speed_up_halting(mask_token, new_halted_tokens_per_layer, percentage, discard_level, patch_width):
    ...
    if discard_level.lower() in {"identity", "id", "i"}:
        ...
        total_indices = true_indices.size(0)
        
        # Calculate the number of True values to retain based on the percentage.
        num_true_to_retain = int(percentage * total_indices)

        # Randomly sample indices
        random_indices = torch.randperm(total_indices)[:num_true_to_retain]

        # Select the subset of true indices using the random indices
        sampled_indices = true_indices[random_indices]
        ...
        if discard_level.lower() in {"nearby", "near", "next", "n"}:
            ...
            mask_token[sampled_indices[left_border_check][:, 0], left_indices[left_border_check][:, 1]] = False
            mask_token[sampled_indices[right_border_check][:, 0], right_indices[right_border_check][:, 1]] = False
        ...
        # 1 --> 5
            ...
            mask_token[sampled_indices[left_border_check][:, 0], left_indices[left_border_check][:, 1]] = False
            mask_token[sampled_indices[right_border_check][:, 0], right_indices[right_border_check][:, 1]] = False
            mask_token[sampled_indices[up_border_check][:, 0], up_indices[up_border_check][:, 1]] = False
            mask_token[sampled_indices[down_border_check][:, 0], down_indices[down_border_check][:, 1]] = False
        ...
        # 1 --> 5
        elif discard_level.lower() in {"dilated-isotropic", "dilated-iso", "dilated", "di-iso", "d-iso"}:
            # Halt the tokens at distant 2 at left, right, up, down, positions
